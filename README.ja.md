...
- 🧠 **多層記憶システム** — 短期・浮動要約・中期（二層要約）・長期(RAG)・意味記憶(Glossary)・根幹記憶
...
3. **モデル割り当て** で Chat / Summary / Gatekeeper / Embedding の各ロールにモデルを選択
   - プリセットから選択するか、**「✏️ カスタム入力...」** を選んで任意のモデル名を直接入力可能
   - 最新APIモデルや旧モデル、Ollama経由のローカルLLMを使いたい場合に便利です
   - Ollamaモデルの場合は `ollama/` プレフィックスを付けてください（例: `ollama/phi3`, `ollama/gemma2`）
4. **💾 モデル設定を保存** をクリック

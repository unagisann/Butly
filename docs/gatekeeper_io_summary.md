...
- **Recent headlines** (`recent_headlines`): Headlines extracted from `recent_digest_headlines.json` (generated by Housekeeper daily batch)
...
SearchPlanner does not receive `recent_headlines` — this is intentional (see design decision in implementation plan).

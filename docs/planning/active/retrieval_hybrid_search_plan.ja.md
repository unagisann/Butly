...
**問ごとの `injection_reason` で帰属を分ける**（temperature 0.7 の生成分散と分離）:

| グループ | n | 平均差 | 合計 | 改善 / 悪化 |
|---|---:|---:|---:|---|
| `candidates` で注入（**policy の効果**） | 16 | **+0.2397** | +3.83 | 6 / 1 |
| `intent`（従来どおり注入。プロンプト不変） | 183 | -0.0065 | -1.19 | — |

cat1-4 の +0.0174 の内訳:

| 要因 | 寄与 |
|---|---:|
| `candidates` policy（cat1-4 12問、+2.83点） | **+0.0186** |
| 分類器のブレ（`need_intent` 180→183。gatekeeper 側の変動で3問が past_fact に） | +0.0104 |
| 生成分散（プロンプトが変わっていない問） | -0.0116 |

**policy 単体の効果は cat1-4 で +0.019、official 199問で +0.019**（+3.83/199）。
事前見積もり +0.024 よりやや小さいが、注入した16問のうち6問が改善・1問が微減
（0.22→0.18）で、方向は明確。§0 で挙げた失敗例のうち
`What pets does Melanie have?` は 0.00→0.57、
`What does Caroline's necklace symbolize?` は 0.00→0.67 と直った
（**BM25 ではなく「検索して注入する」だけで直った**）。
`What types of pottery have Melanie and her kids made?` は注入されても 0.00 のままで、
読み手側の問題として残る。

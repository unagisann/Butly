...
表示するのは設定済みかどうかだけである。APIキー、Connection追加、Ollama URLなど
独立して送信できる入力はStreamlit form内にあり、入力途中では画面を再実行しない。
設定画面自体もfragmentで、Connection→モデルの連動選択は設定画面内だけを再描画する。
非選択セクションは実行されず、そのセクション専用APIも取得しない。

...
| `schemas/` | Frontend DTOs: `common.py` (ApiError), `system.py`, `chat.py` (chat/history/SSE/debug/request status), `instances.py`, `preflight.py`, and `trace.py` (Trace Graph: returns only the Mermaid string and per-status node counts, never `TraceNode.metadata` — issue #51). |

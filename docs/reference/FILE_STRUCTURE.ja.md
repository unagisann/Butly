...
| `src/lifecycle/` | Tauri command / event を隠す bridge。dev の browser 実行では dev server が proxy する同一 origin を見る DevBrowserBridge、production の Tauri 外では NullBridge（[起動手順](../guides/desktop_dev_setup.ja.md)） |

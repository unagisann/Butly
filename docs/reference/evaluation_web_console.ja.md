...
再実行しない。モデル候補はrole・Connection単位で10分キャッシュされ、未取得の
Connectionを選んだ場合だけ取得する。「モデル一覧を更新」はキャッシュを明示的に
破棄する。新規LoCoMo評価フォームと日本語対話A/Bフォームも
それぞれfragmentとして分離し、フォーム内の選択変更では画面全体やinstance一覧を
再取得しない。評価configとrun一覧は10秒の短時間キャッシュを使い、開始操作では
直ちに無効化する。

...
14. **Semantic Judge自身が2026-08-16 18:40に変わっており、それをまたぐ比較は無効だった。**
    ...
    **0/87で完全に一致**したので、これは反転ノイズではなく経路変更の系統差である。
    ...
    測定対象の変更と同じ厳しさで記録する。**
15. **検索が完璧でもcat1-4は0.63で止まる。伸びしろは検索側に+0.17しかない。** gold evidence
    だけを注入するOracle Reader（v41）は、v40と同一カード・同一dataset・同一Judge経路で
    cat1-4を0.4524→0.6258（t=16.84）、Semanticで0.6175→0.7753（t=12.48）まで上げた。
    しかし**1,540問中262問（17.0%）は根拠が揃っていても外す**。これが`utilization_miss`の
    床である。さらに段階別に分解すると、**伸びしろの41%は「カードが根拠を指していない」
    （provenance）、32%は「候補に居るのにtop3へ来ない」（ランキング）、19%は候補top20落ち**で、
    合わせて92%が「原文はあるのに正しいものを引けていない」に帰する。
    **渡し方は課題ではない**: 原文が全件届いた833問ではv40の0.859がv41の0.832を上回り、
    RAW3が近傍ごと渡す「余分な発話」はノイズではなく利益だった。渡し方で残るのは
    「一部しか届かない」196問（寄与15%、うち154問がcat1）で、質ではなく量の問題である。
    cat5は-0.11（t=-3.78）と下がるが、これはLoCoMoが「情報なし」が正解の問にも
    evidenceを付けているためで、oracle注入の欠陥ではない（5.12）。
...
### 5.12 v41 Oracle Reader: 検索を外して読解の上限を測った
...
したがってこのrunは**「検索が完璧だったとき読解はどこまで行くか」の上限**にあたる。
...
cat1-4の1,540問のうち262問（17.0%）はSemanticで`incorrect`のままである。
これは検索でもカード化でも注入でも説明できない、純粋な
[`utilization_miss`](../planning/active/raw_injection_plan.ja.md)の床である。

したがって「検索を直せば解ける」と言える余地はcat1-4で**+0.17まで**であり、
そこから先はreader側（プロンプト、モデル、回答形式）の課題になる。

#### cat5の低下は注入の副作用ではなくLoCoMo注記の性質

cat5は-0.1099（t=-3.78）と、5.7以降で警戒してきた方向へはっきり動いた。
ただしこれはoracle注入が悪いのではなく、**LoCoMoが「No information available」が正解の問にも
evidenceターンを付けている**ことによる。指し先は正解に似た別の発話なので、
それを確実に渡すほどモデルは棄権をやめる。
...
**よってv41を読解上限として読むときはcat1-4を使う。** cat5込みのoverall 0.5925は
この構造的な負荷を含んだ数字である。cat5の位置づけ（副作用の監視指標）は5.7から変えない。

#### 伸びしろがどの段階にあるか

v41があるので「各段階を完全に直したら回答がどれだけ増えるか」を段階別に出せる。
cat1-4の1,531問（provenanceを解決できた問）を、**gold evidenceの原文が実際にpromptへ
入ったか**で排他分類し、各層のv41スコアをその段階を直したときの上限として当てる。
...
#### 渡し方は問題ではない。近傍を混ぜるほうが強い

**段階6が決定的である。原文が全件届いた833問では、v40の0.859がv41の0.832を上回る。**

RAW3は`source_files`をファイル粒度で近傍ごと渡すため、gold以外の発話も混ざる。
その「余分な発話」はノイズではなく**利益**だった。goldターンだけを純粋に渡すoracleのほうが
負けている。[RAW注入の見直し計画](../planning/active/raw_injection_plan.ja.md)§5の
「会話ログでは最も似た断片より近傍ごとのほうが当たる」（passage単体へ置き換えると-202問）が、
検索指標だけでなく回答スコアでも再現したことになる。

渡し方で残るのは段階5「一部しか届かない」196問（寄与15%）だけで、これは中身の質ではなく
**量**の問題である。この層は154/196がcat1（multi-hop）で必要evidenceが平均3.50ターン、
段階6の1.12ターンと対照的である。

#### 換算率は届け方の精度ではなく完全性で決まる
...
| Oracle Reader | 検索を全て止め、gold evidenceが指す原文ターンだけを注入する評価専用モード（`rerun-qa --qa-memory-mode oracle`） | gold answerは渡さない。読解上限の測定用で、本番経路には公開しない。cat5はLoCoMo注記の性質上不利になる |
| `utilization_miss` | 根拠がpromptに入っているのに答えられない失敗 | Oracle Readerで直接測る。v41のcat1-4では262/1,540問（17.0%） |

...
評価系列では2026-08-16時点でtop-5が優位だが、本番既定の変更はtop-10の結果を見てから判断する。
...
| vector注入上限 | 3 | 2 / 3 / 5をA/B | 現行3。評価系列はv38で5が優位（cat1-4 +1.11pt、Semantic +1.72pt、cat5据え置き）。top-10の結果を見て本番既定を判断 |
...
9. **運用耐久指標を追加する。** retry件数、最終失敗率、artifact hash、同一run再実行時の
   上書き防止を記録する。
10. **注入top-kの曲線を確定する（次の1本）。** v37のカードを再利用し
    `vector_search_limit=10`だけを変えたrunを作る。Recall@3 0.707に対しRecall@20が0.856あり、
    3→5で拾えたのは4〜20位帯の一部にすぎない。この1本で「傾きが本物か」と
    「cat5とcontradictionがどこで崩れ始めるか」を同時に見る。5.7と同じJudge設定
    （`google/gemma-4-31b-it` / `nanogpt-sub` / temperature 0）で必ず判定まで回す。
